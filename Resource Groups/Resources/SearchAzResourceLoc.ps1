...
<# Variables: {
    :SearchAzureRSByLoc         Outer loop for managing function
    :SelectAzureResource        Inner loop for selecting a resource
    $CallingFunction:           Name of this function
    $LocationObject:            Azure location object
    $RSObject:                  Resource object
    $ListNumber:                $var used for number in $ListArray
    $ListArray:                 Array used to list and select the resource
    $ListInput:                 $var used to load info into $ListArray
    $Number:                    Current item .Number
    $RSSelect:                  Operator input to select item from $ListArray
} #>
<# Process Flow {
    Function               
