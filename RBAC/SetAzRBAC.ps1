...
function FunctionName { # This function is used to create or replace roles for a user
    Begin {
        ...
        $RightsLevel = Read-Host "Role (Type 'none' if all roles are to be removed)" # Operator input for role (Not sanitized)
        ...
        if ($Scope -eq "y" -or $Scope -eq "Yes") { # Verify operator input
            ...
            if ($RightsLevel -eq "None") { # None ends the script without assigning new rights
